    ...
    <div class="modal-body">
        <div class="row">
            <div class="form-group col-md-6">
                ...
                <select type="text" name="supplier" id="supplier" class="form-control" required="required">
            		<option value="">-- Pilih Supplier --</option>
				<?php
					$master	= $conn->prepare("SELECT id_sup, kode_sup, nama_sup FROM supplier ORDER BY nama_sup ASC");
					$master->execute();
					while($hasil= $master->fetch(PDO::FETCH_ASSOC)){
						$pilih	= (@$pecah[0]==$hasil['id_sup']) ? 'selected="selected"' : '';
				?>
                	<option value="<?php echo($hasil['id_sup']); ?>" <?php echo($pilih); ?>><?php echo($hasil['nama_sup']); ?></option>
                <?php } ?>
                </select>
            </div>
            <div class="form-group col-md-6">
...
?>
	...
	$("#btncari").click(function(){
		...
		var tgl1	= $("#tgl1").val();
		var tgl2	= $("#tgl2").val();
		var gabung	= "cari="+supplier+"_"+obat+"_"+tgl1+"_"+tgl2;
