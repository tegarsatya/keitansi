<?php
	...
	switch($act){
		case "input":
			$kode	= $data->bcode('RKB', 'id_rkb', 'regional_kabupaten');
			$prov	= $secu->injection($_POST['provinsi']);
			$nama	= $secu->injection($_POST['nama']);
			$save	= $conn->prepare("INSERT INTO regional_kabupaten VALUES(:prov, :kode, :nama, :catat, :admin, :catat, :admin)");
		...
		$prov	= $secu->injection($_POST['provinsi']);
			$nama	= $secu->injection($_POST['nama']);
			$edit	= $conn->prepare("UPDATE regional_kabupaten SET id_rpo=:prov, nama_rkb=:nama, updated_at=:catat, updated_by=:admin WHERE id_rkb=:kode");
		...
		case "delete":
			$kode	= $secu->injection($_POST['keycode']);
			$dele	= $conn->prepare("DELETE FROM regional_kabupaten WHERE id_rkb=:kode");
