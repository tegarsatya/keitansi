<?php
	...
	switch($act){
		case "input":
			$kode	= $data->bcode('KLG', 'id_klg', 'kategori_legal');
			$nama	= $secu->injection($_POST['nama']);
			$bulan	= $secu->injection($_POST['bulan']);
			$notif	= $secu->injection($_POST['notif']);
			$save	= $conn->prepare("INSERT INTO kategori_legal VALUES(:kode, :nama, :bulan, :notif, :catat, :admin, :catat, :admin)");
		...
		case "update":
			...
			$bulan	= $secu->injection($_POST['bulan']);
			$notif	= $secu->injection($_POST['notif']);
			$edit	= $conn->prepare("UPDATE kategori_legal SET nama_klg=:nama, parameter_klg=:bulan, notif_klg=:notif, updated_at=:catat, updated_by=:admin WHERE id_klg=:kode");
		...
		case "delete":
			$kode	= $secu->injection($_POST['keycode']);
			$dele	= $conn->prepare("DELETE FROM kategori_legal WHERE id_klg=:kode");
