<?php
	...
	switch($act){
		case "input":
			$kode	= $data->bcode('KPR', 'id_kpr', 'kategori_produk');
			$nama	= $secu->injection($_POST['nama']);
			$satuan	= $secu->injection($_POST['satuan']);
			$save	= $conn->prepare("INSERT INTO kategori_produk VALUES(:kode, :nama, :satuan, :catat, :admin, :catat, :admin)");
		...
		case "update":
			...
			$nama	= $secu->injection($_POST['nama']);
			$satuan	= $secu->injection($_POST['satuan']);
			$edit	= $conn->prepare("UPDATE kategori_produk SET nama_kpr=:nama, satuan_kpr=:satuan, updated_at=:catat, updated_by=:admin WHERE id_kpr=:kode");
		...
		case "delete":
			$kode	= $secu->injection($_POST['keycode']);
			$dele	= $conn->prepare("DELETE FROM kategori_produk WHERE id_kpr=:kode");
