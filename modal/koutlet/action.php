<?php
	...
	switch($act){
		case "input":
			$kode	= $data->bcode('KOT', 'id_kot', 'kategori_outlet');
			$nama	= $secu->injection($_POST['nama']);
			$code	= $secu->injection($_POST['code']);
			$save	= $conn->prepare("INSERT INTO kategori_outlet VALUES(:kode, :nama, :code, :catat, :admin, :catat, :admin)");
		...
		case "update":
			...
			$nama	= $secu->injection($_POST['nama']);
			$code	= $secu->injection($_POST['code']);
			$edit	= $conn->prepare("UPDATE kategori_outlet SET nama_kot=:nama, kode_kot=:code, updated_at=:catat, updated_by=:admin WHERE id_kot=:kode");
		...
		case "delete":
			$kode	= $secu->injection($_POST['keycode']);
			$dele	= $conn->prepare("DELETE FROM kategori_outlet WHERE id_kot=:kode");
