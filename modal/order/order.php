<?php
	...
	switch($modal){
		...
		$read	= $conn->prepare("SELECT kode_tor, tgl_tor FROM transaksi_order WHERE id_tor=:kode");
...
?>
        ...
        <div class="modal-body">
            ...
            <div class="row">
                <div class="form-group col-md-12">
                    <label>Tanggal Order <span class="tx-danger">*</span></label>
                    <input type="text" name="tgl_tor" class="form-control fortgl" value="<?php echo($view['tgl_tor']); ?>" placeholder="9999-99-99" required="required" />
                    <div id="imgloading"></div>
                </div>
			</div>
        </div>
...
<?php
		...
		case 'delete':
		$kode	= $secu->injection($_GET['keycode']);
		$read	= $conn->prepare("SELECT status_tor FROM transaksi_order WHERE id_tor=:kode");
		$read->bindParam(':kode', $kode, PDO::PARAM_STR);
		$read->execute();
		$view	= $read->fetch(PDO::FETCH_ASSOC);
?>
...
<?php
		...
		case 'tutup':
		$kode	= $secu->injection($_GET['keycode']);
		$read	= $conn->prepare("SELECT kode_tor, proses_tor FROM transaksi_order WHERE id_tor=:kode");
		$read->bindParam(':kode', $kode, PDO::PARAM_STR);
		$read->execute();
		$view	= $read->fetch(PDO::FETCH_ASSOC);
?>
