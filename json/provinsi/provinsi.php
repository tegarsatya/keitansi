<?php
	...
	if($valid==false){
		...
		$tabel	= '';
		$no		= $mulai;
		$jumlah	= $conn->query("SELECT COUNT(id_rpo) AS total FROM regional_provinsi WHERE nama_rpo LIKE '%$cari%'")->fetch(PDO::FETCH_ASSOC);
		$master	= $conn->prepare("SELECT id_rpo, nama_rpo FROM regional_provinsi WHERE nama_rpo LIKE '%$cari%' ORDER BY nama_rpo ASC LIMIT :mulai, :maxi");
