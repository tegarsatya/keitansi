<?php
	...
	if($valid==false){
		...
		$tabel	= '';
		$no		= $mulai;
		$jumlah	= $conn->query("SELECT COUNT(A.id_rkb) AS total FROM regional_kabupaten AS A INNER JOIN regional_provinsi AS B ON A.id_rpo=B.id_rpo WHERE A.nama_rkb LIKE '%$cari%' OR B.nama_rpo LIKE '%$cari%'")->fetch(PDO::FETCH_ASSOC);
		$master	= $conn->prepare("SELECT A.id_rkb, A.nama_rkb, B.nama_rpo FROM regional_kabupaten AS A INNER JOIN regional_provinsi AS B ON A.id_rpo=B.id_rpo WHERE A.nama_rkb LIKE '%$cari%' OR B.nama_rpo LIKE '%$cari%' ORDER BY B.nama_rpo ASC, A.nama_rkb LIMIT :mulai, :maxi");
