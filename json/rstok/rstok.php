<?php
	...
	if($valid==false){
		...
		if(empty($tgl1) || empty($tgl2)){
			...
			$tabel	= '';
			$no		= $mulai;
			$jumlah	= $conn->query("SELECT COUNT(id_pro) AS total FROM produk WHERE nama_pro LIKE '%$search%'")->fetch(PDO::FETCH_ASSOC);
			$master	= $conn->prepare("SELECT id_pro, nama_pro FROM produk WHERE nama_pro LIKE '%$search%' ORDER BY nama_pro ASC LIMIT :mulai, :maxi");
