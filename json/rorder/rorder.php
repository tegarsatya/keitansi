<?php
	...
	if($valid==false){
		...
		$tabel	= '';
		$no		= $mulai;
		$jumlah	= $conn->query("SELECT COUNT(TABEL.id_tre) AS total FROM(SELECT A.id_tre FROM transaksi_receive AS A INNER JOIN supplier AS B ON A.id_sup=B.id_sup INNER JOIN transaksi_order AS C ON A.id_tor=C.id_tor LEFT JOIN transaksi_receivedetail AS D ON A.id_tre=D.id_tre LEFT JOIN produk AS E ON D.id_pro=E.id_pro WHERE A.fak_tre LIKE '%$cari%' OR C.kode_tor LIKE '%$cari%' OR B.nama_sup LIKE '%$cari%' OR E.nama_pro LIKE '%$cari%' GROUP BY A.id_tre) AS TABEL")->fetch(PDO::FETCH_ASSOC);
		$master	= $conn->prepare("SELECT A.id_tre, A.fak_tre, A.tglfak_tre, A.tgl_tre, A.total_tre, A.tgl_limit, A.status_tre, B.nama_sup, C.kode_tor FROM transaksi_receive AS A INNER JOIN supplier AS B ON A.id_sup=B.id_sup INNER JOIN transaksi_order AS C ON A.id_tor=C.id_tor LEFT JOIN transaksi_receivedetail AS D ON A.id_tre=D.id_tre LEFT JOIN produk AS E ON D.id_pro=E.id_pro WHERE A.fak_tre LIKE '%$cari%' OR C.kode_tor LIKE '%$cari%' OR B.nama_sup LIKE '%$cari%' OR E.nama_pro LIKE '%$cari%' GROUP BY A.id_tre ORDER BY A.tgl_tre DESC LIMIT :mulai, :maxi");
