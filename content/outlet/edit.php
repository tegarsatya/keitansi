...
<?php
	...
	$read	= $conn->prepare("SELECT A.id_kot, A.nama_out, A.resmi_out, A.npwp_out, A.ofcode_out, A.ket_out, B.id_rpo, B.id_rkb, B.telp_ola, B.hp_ola, B.fax_ola, B.email_ola, B.web_ola, B.kopos_ola, B.picp_ola, B.picpk_ola, B.picf_ola, B.picfk_ola, B.jatuk_ola, B.kantor_ola, B.pengiriman_ola, B.atuk_ola, C.top_odi, C.parameter_odi, C.diskon_odi, COUNT(D.id_ole) AS legal FROM outlet AS A INNER JOIN outlet_alamat AS B ON A.id_out=B.id_out INNER JOIN outlet_diskon AS C ON A.id_out=C.id_out LEFT JOIN outlet_legal AS D ON A.id_out=D.id_out WHERE A.id_out=:kode GROUP BY A.id_out");
